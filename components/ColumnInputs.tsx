...
import { UploadCloud, FileText, CheckCircle, AlertCircle, Trash2, Eye, Sparkles } from 'lucide-react';
import { useStore } from '../store';
import { InputTab, FileData } from '../types';
import { detectarMateria, detectarMateriasRanking } from '../services/materiaDetector';
...
const MATERIA_NAMES: Record<string, string> = {
  ...
  multa: 'Pena de Multa',
  falta: 'Falta Grave/Regressão',
  agravo: 'Agravo',
  reconsideracao: 'Reconsideração',
};

const ColumnInputs: React.FC = () => {
  const { inputs, activeInputTab, setActiveInputTab, setInput, files, addFile, detectedMatter, setDetectedMatter } = useStore();
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Detectar matéria automaticamente quando o texto muda
  useEffect(() => {
    const textoCompleto = `${inputs.rspe} ${inputs.mp} ${inputs.defesa} ${inputs.outros}`.trim();
    if (textoCompleto.length > 20) {
      const materiaDetectada = detectarMateria(textoCompleto);
      setDetectedMatter(materiaDetectada);
    } else {
      setDetectedMatter(null);
    }
  }, [inputs.rspe, inputs.mp, inputs.defesa, inputs.outros, setDetectedMatter]);
  ...
  const renderTabContent = () => {
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        {/* Indicador de Matéria Detectada */}
        {detectedMatter && (
          <div className="mt-3 flex items-center gap-2 p-2 bg-accent/5 border border-accent/20 rounded-lg">
            <Sparkles size={14} className="text-accent" />
            <span className="text-xs text-slate-600">
              Matéria detectada: <strong className="text-accent">{MATERIA_NAMES[detectedMatter] || detectedMatter}</strong>
            </span>
          </div>
        )}
      </div>
