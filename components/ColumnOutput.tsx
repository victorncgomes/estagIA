...
// Mapeamento de nomes amigáveis dos modelos
...
const ColumnOutput: React.FC = () => {
    const { output, setActiveOutputTab, selectedModel } = useStore();
    const { logs, activeOutputTab, text, prompt, progress, isGenerating } = output;

    // Obtém nome amigável do modelo ativo
    const getActiveModelName = (): string => {
        const available = getAvailableProviders();
        if (available.length === 0) return 'Nenhum configurado';
        const activeProvider = available.includes(selectedModel as any) ? selectedModel : available[0];
        return MODEL_NAMES[activeProvider] || activeProvider;
    };
        ...
        return (
            <div className="h-full flex flex-col">
                <div className="flex-1 relative">
                    <textarea
                        className="w-full h-full p-6 bg-white border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-accent resize-none font-serif leading-loose shadow-sm"
                        value={text}
        ...
        <div className="flex flex-col h-full bg-slate-50">
            ...
            <div className="px-4 py-2 border-t border-slate-200 bg-white flex justify-between items-center text-[10px] text-slate-400">
                <div className="flex items-center gap-2">
                    ...
                    <span>Modelo: {getActiveModelName()}</span>
                </div>
                <div>
